#' @title Time-course Metabolomic Study with dataset with QCSs and the corresponding data of golden standards for performance evaluation using Criterion e.
...
#' @usage nortimecourseqcallgs(fileName, GS, SAalpha="Y", SAbeta="Y", SAgamma="Y")
...
nortimecourseqcallgs <- function(fileName, GS, SAalpha="Y", SAbeta="Y", SAgamma="Y"){
  ...
  # clean useless data
  rm(afterqc.table,afterqc_table,data1,data2,
  ...
  k.test <- foreach::foreach (k.input = iterators::iter(kk),.options.snow=opts,.packages=c("reshape","ggplot2"),.combine = "rbind") %dopar% {  # ,.errorhandling = c("pass")
    ...
    dir.create(paste0("OUTPUT-NOREVA-Criteria.Ce"))
    pdf(file=paste("./OUTPUT-NOREVA-Criteria.Ce/Criteria.Ce-",k.step2.name,".pdf",sep=""))
    ...
    k.temp <- c(k.step2.name,mean(pmad3N),accuracy,CW_value,AUC_mic,abs(median(result,na.rm=TRUE)))
    return(k.temp)
