...
#' @description this function enables the performance assessment of metabolomic data processing for
#' the time-course dataset (with quality control sample but without internal standard)
#' using four independent criteria, and can comprehensively scan thousands of processing workflows
#' and rank all these workflows based on their performances (assessed from four different perspectives).
...
#' @usage nortimecourseqcall(fileName, SAalpha="Y", SAbeta="Y", SAgamma="Y")
#' @export nortimecourseqcall
#' @examples
#' library(NOREVA)
#' \donttest{timec_qc_data <- PrepareInuputFiles(dataformat = 1,
#' rawdata = "Timecourse_with_QCS.csv")}
#' \donttest{nortimecourseqcall(fileName = timec_qc_data,
#' SAalpha="Y", SAbeta="Y", SAgamma="Y")}

nortimecourseqcall <- function(fileName, SAalpha="Y", SAbeta="Y", SAgamma="Y"){
  ...
  cat("\n")
  cat("STEP 2: QC Sample Correction and Parameter Setting of Regression Model", "\n")
  cat("\n")

  cat("STEP 3: The assumption(s) held as indicated by users","\n")
  ...
  cat("\n")
  cat("STEP 4: The criteira selected by users for this assessment","\n")
...
#data_q <- read.csv("Data_Time-course_with_QCS.csv", header = TRUE,stringsAsFactors = FALSE)
  ...
  k.l <-    foreach (i = 1:4,.combine = "c") %do%{
    ...
    #for (j in as.numeric(trsf)){
      ...
      ####
        return(list(i,q,j,train_data_Transformation=train_data_Transformation,afterqc.table=afterqc.table,sampleLabel=sampleLabel,afterqc.trans=afterqc.trans))
    }
  }
  }
  ## calculate
    ...
    if(any(!is.na(normal1))){
      ...
      #{ next }
        ...
        tryCatch({
          ...
          train_data_metabolite3<-norm_sam(train_data_Preprocess3,h)
          ...
          #}else{
          set.seed(3)
          normalized_data3 <- t(train_data_metabolite3)
        ...
        k.b[[paste(im_nam[i], q_nam[q], t_nam[j],n_nam[k],n_nam[h],sep="+")]] <- afterqc.trans
  ...
  if(any(!is.na(normal5))){
    k.n3 <- foreach(k = normal5) %do%{
      ...
      tryCatch({
        set.seed(3)
        train_data_metabolite3<-norm_sam(train_data_Transformation,k)
        #if(class(train_data_metabolite3)=="try-error")
        #{ next }
      ...
      k.b[[paste(im_nam[i],q_nam[q],t_nam[j],n_nam[k],sep="+")]] <- afterqc.trans
     # save(normal_data,file="./OUTPUT-NOREVA-All.Normalized.Data.Rdata")
      return(list(k.a,k.b))
    }}
...
#    Fpurity[names(normal_data[mmm])]<-accuracy
  ...
  dev.off()
    ...
    tryCatch({
    DEG.list <- consistency(3, 75)}, error=function(e){})
  ...
  save(k.test,file="./step2_data.Rdata") # rm(k.test,k.result1,k.result2,result2)
