...
#' GS = "Multiclass_with_QCS_GoldenStandard.csv", SAalpha="Y", SAbeta="Y", SAgamma="N")}
  ...
  #for (i in as.numeric(impt)){
    ...
    #for (j in as.numeric(trsf)){
      ...
      for(k in normal5){
        ...
        eva_data3 <- NULL
        aftetable[[paste(im_nam[i], q_nam[q], t_nam[j], paste("[",paste(n_nam[k]),"]", sep=""), sep="+")]] <- afterqc.table
        save(normal_data,file="./OUTPUT-NOREVA-All.Normalized.Data.Rdata")
      }}
    }
    }
