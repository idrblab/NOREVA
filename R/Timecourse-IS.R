...
#' for time-course dataset (with internal standards but without quality control sample) using
#' four independent criteria, and can comprehensively scan thousands of processing workflows and
#' rank all these workflows based on their performances (assessed from four different perspectives).
...
#' For example, the value of argument IS that is set to “2,6,8,n” indicates that the metabolites in the 3rd, 7th, 9th,
#' and (n+1)th columns of your input peak table should be considered to be the IS metabolites.
...
#' @usage nortimecourseisall(fileName, IS)
#' @export nortimecourseisall
#' @examples
#' library(NOREVA)
#' \donttest{timec_is_data <- PrepareInuputFiles(dataformat = 1,
#' rawdata = "Timecourse_with_IS.csv")}
#' \donttest{nortimecourseisall(fileName = timec_is_data, IS = "2,6,9")}

nortimecourseisall <- function(fileName, IS){
...
for (mmm in 1:length(normal_data)){
  ...
  for (mm in 1:5) {
    ...
    tryCatch({
    svmmodel <- svm(Label ~ ., data = train_df, probability = TRUE)}, error=function(e){})
