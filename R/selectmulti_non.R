...
#' processing for multi-class dataset (without quality control sample and
#' without internal standard) using four criteria, and can scan the customized workflows selected
#' by user and rank them based on performances.
...
#' @usage normulticlassnopart(fileName, selectedMethods)
#' @export normulticlassnopart
#' @examples
#' library(NOREVA)
#' \donttest{multi_non_data <- PrepareInuputFiles(dataformat = 1,
#' rawdata = "Multiclass_without_QCSIS.csv")}
#' \donttest{normulticlassnopart(fileName = multi_non_data, selectedMethods = "selectedmethods.csv")}

normulticlassnopart <- function(fileName, selectedMethods){
  ...
  for (mmm in 1:length(normal_data)){
    ...
    tryCatch({
    DEG.list <- consistency_M(3, 10)}, error=function(e){})
