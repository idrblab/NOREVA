#' @title Multi-class (N>1) Metabolomic Study with dataset with Quality Control Samples (QCSs)
#' @description This function enables the performance assessment of metabolomic data
#' processing for multi-class dataset (with quality control sample but without internal
#' standard) using four criteria, and can scan thousands of processing
...
#' @import ggrepel ggpubr sampling crmn foreach dplyr iterators
...
#' @usage normulticlassqcall(fileName, SAalpha="Y", SAbeta="Y", SAgamma="Y")
...
normulticlassqcall <- function(fileName, SAalpha="Y", SAbeta="Y", SAgamma="Y"){
 ...
 # nanmes_right<-names(normal_data)
  ...
  k.test <- foreach::foreach (k.input = iterators::iter(kk),.options.snow=opts,.packages=c("reshape","ggplot2"),.combine = "rbind") %dopar% {  # ,.errorhandling = c("pass")
    ...
    #################### output
    ##########################################################################################
    # Fpmad[k.step2.name]<-mean(pmad3N) # output
    ##########################################################################################
    #################### output
    ##########################################################################################
    ...
    #################### output
    ##########################################################################################
    # Fscore[k.step2.name]<-CW_value # output-Fscore[k.step2.name]
    ...
    #################### output
    ##########################################################################################
    # Fauc[k.step2.name]<-auc.value # output-Fauc[k.step2.name]
